...
function PreparedDataShowcase() {
  ...
  const selected = PREPARED_SESSION_MANIFEST.sessions.find(
    (session) => session.id === selectedSessionId,
  ) ?? PREPARED_SESSION_MANIFEST.sessions[0];

  return (
    <main className="min-h-screen bg-zinc-950 px-4 py-10 text-zinc-100 sm:px-6">
      <section className="mx-auto max-w-6xl">
        <p className="text-xs font-medium uppercase tracking-[0.22em] text-emerald-400">Prepared DataCaster sessions</p>
        <h1 className="mt-3 max-w-4xl text-4xl font-semibold tracking-tight sm:text-5xl">Explore a finished match-analysis journey before a live run.</h1>
        <p className="mt-4 max-w-3xl text-base leading-7 text-zinc-400">
          Choose a frozen scouting session, inspect its timeline and timestamped answer, then watch the
          recorded proof point. These client-side records are fast to switch and never ingest, query, or
          generate new VideoDB data.
        </p>
        <div className="mt-6 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm leading-6 text-amber-100">
          <strong>Prepared provenance:</strong> {PREPARED_SESSION_MANIFEST.provenance}
        </div>

        <div className="mt-8 grid gap-5 lg:grid-cols-[290px_minmax(0,1fr)]">
          <nav className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-3" aria-label="Prepared match sessions">
            <p className="px-2 pb-2 pt-1 text-xs font-medium uppercase tracking-[0.16em] text-emerald-400">Prepared examples</p>
            <div className="grid gap-2">
              {PREPARED_SESSION_MANIFEST.sessions.map((session) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl border p-4 text-left transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-400 ${
                      active
                        ? "border-emerald-400/50 bg-emerald-400/10"
                        : "border-zinc-800 bg-zinc-950/50 hover:border-zinc-700 hover:bg-zinc-900"
                    }`}
                  >
                    <span className="block text-xs font-medium uppercase tracking-[0.15em] text-emerald-400">{session.label}</span>
                    <span className="mt-2 block text-sm font-medium text-zinc-100">{session.title}</span>
                    <span className="mt-2 block text-xs leading-5 text-zinc-400">{session.summary}</span>
                  </button>
                );
              })}
            </div>
          ...
          <article className="min-w-0 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 sm:p-6" aria-live="polite">
            <div className="flex flex-col gap-4 border-b border-zinc-800 pb-5 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <p className="text-xs font-medium uppercase tracking-[0.16em] text-emerald-400">{selected.label} - frozen match-analysis session</p>
                <h2 className="mt-2 text-2xl font-semibold tracking-tight text-zinc-50">{selected.title}</h2>
                <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400">{selected.summary}</p>
              </div>
              <a
                className="shrink-0 rounded-lg border border-zinc-700 px-3 py-2 text-xs font-medium text-zinc-200 transition hover:border-emerald-400/60 hover:text-emerald-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-400"
            ...
            <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.06fr)_minmax(280px,.94fr)]">
              <section aria-labelledby="timeline-heading">
                <p className="text-xs font-medium uppercase tracking-[0.16em] text-emerald-400">Step 1 - frozen event timeline</p>
                <h3 id="timeline-heading" className="mt-2 text-lg font-medium text-zinc-100">{selected.sourceLabel}</h3>
                <ol className="mt-4 grid gap-2">
                  {selected.timeline.map((event) => (
                    <li key={`${event.time}-${event.kind}`} className="grid grid-cols-[66px_minmax(0,1fr)] gap-3 rounded-lg border border-zinc-800 bg-zinc-950/60 p-3">
                      <span className="font-mono text-xs font-medium text-emerald-300">{event.time}</span>
                      <span>
                        <strong className="block text-sm font-medium text-zinc-100">{event.kind}</strong>
                        <span className="mt-1 block text-xs leading-5 text-zinc-400">{event.detail}</span>
                      </span>
                    </li>
                  ))}
                </ol>
              ...
              <section className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4" aria-labelledby="ask-heading">
                <p className="text-xs font-medium uppercase tracking-[0.16em] text-emerald-400">Step 2 - evidence-backed Ask</p>
                <h3 id="ask-heading" className="mt-2 text-base font-medium text-zinc-100">{selected.question}</h3>
                <p className="mt-3 text-sm leading-6 text-zinc-300">{selected.answer}</p>
                <ul className="mt-4 grid gap-2 border-t border-emerald-500/20 pt-4 text-xs leading-5 text-zinc-400">
                  {selected.evidence.map((item) => <li key={item}>• {item}</li>)}
                </ul>
                <p className="mt-4 text-xs leading-5 text-zinc-500">The answer and citations are frozen walkthrough fixtures, not a response generated by this deployment.</p>
              </section>
            </div>

            <section className="mt-6 border-t border-zinc-800 pt-6" aria-labelledby="reel-heading">
              <p className="text-xs font-medium uppercase tracking-[0.16em] text-emerald-400">Step 3 - highlight handoff</p>
              <h3 id="reel-heading" className="mt-2 text-lg font-medium text-zinc-100">{selected.playbackLabel}</h3>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-400">{selected.reelNote}</p>
              <div className="mt-4 aspect-video overflow-hidden rounded-xl border border-zinc-800 bg-black shadow-2xl">
                <iframe
                  className="h-full w-full"
                  src={selected.playbackUrl}
                  title={selected.playbackLabel}
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                  allowFullScreen
                />
              </div>
              <p className="mt-3 text-xs leading-5 text-zinc-500">
                If the embedded player is unavailable, <a className="text-emerald-300 underline underline-offset-2" href="https://www.youtube.com/watch?v=lR99z0Jel-4" target="_blank" rel="noreferrer">open the recorded walkthrough on YouTube</a>.
              </p>
            </section>
          </article>
        </div>
