  ...
  const preparedRecords = [
    {
      ...
      detail: "The recorded walkthrough filters goal events and seeks the player to a selected moment.",
    },
    {
      label: "Ask with evidence",
      timestamp: "01:10 - 01:25",
      detail: "The recorded answer cites [00:18] and [04:24] as its supporting match moments.",
    },
    {
      label: "Prepared highlight",
      timestamp: "00:50 - 01:10",
      detail: "The walkthrough shows the project’s Timeline-composed vertical reel preview.",
    },
  ];
  ...
  return (
    <main className="min-h-screen bg-zinc-950 px-4 py-10 text-zinc-100">
      <section className="mx-auto max-w-5xl">
        <p className="text-xs font-medium uppercase tracking-[0.22em] text-emerald-400">Prepared DataCaster session</p>
        <h1 className="mt-3 text-4xl font-semibold tracking-tight">Review a recorded VideoDB match-analysis session.</h1>
        <p className="mt-4 max-w-3xl text-base leading-7 text-zinc-400">
          This read-only Vercel deployment presents retained walkthrough evidence from the
          project’s submitted DataCaster demo. The cards below are a prepared reading guide -
          they do not query, ingest, or generate new VideoDB data.
        </p>

        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {preparedRecords.map((record) => (
            <article key={record.label} className="rounded-lg border border-zinc-800 bg-zinc-900/60 p-5">
              <p className="text-xs font-medium uppercase tracking-[0.16em] text-emerald-400">Prepared record</p>
              <h2 className="mt-3 text-lg font-medium text-zinc-100">{record.label}</h2>
              <p className="mt-2 font-mono text-sm text-emerald-300">{record.timestamp}</p>
              <p className="mt-3 text-sm leading-6 text-zinc-400">{record.detail}</p>
            </article>
          ))}
        </div>

        <section className="mt-8 rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
          <p className="text-xs font-medium uppercase tracking-[0.16em] text-emerald-400">Prepared Ask result</p>
          <h2 className="mt-3 text-xl font-medium">Did anyone get a red card?</h2>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-300">
            <span className="font-medium text-zinc-100">Recorded demo answer:</span> Yes - two reds. [00:18]
            referee shows red to a defender after a last-man tackle; [04:24] keeper sent off for
            handling outside the box.
          </p>
          <p className="mt-3 text-xs leading-5 text-zinc-500">
            Timestamped evidence is reproduced from the project’s recorded demo/API example, not
            generated by this deployment.
          </p>
        </section>
        ...
        <div className="mt-8 aspect-video overflow-hidden rounded-xl border border-zinc-800 bg-black shadow-2xl">
          <iframe
            ...
            src="https://www.youtube-nocookie.com/embed/lR99z0Jel-4?start=50&end=70"
            title="Recorded DataCaster prepared highlight evidence"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          />
        </div>
        <p className="mt-3 text-xs leading-5 text-zinc-500">
          Prepared playback is the submitted walkthrough’s reel segment. No standalone exported
          VideoDB highlight URL is retained in this repository, so this deployment does not claim
          that the embedded video is a newly generated reel.
        </p>
