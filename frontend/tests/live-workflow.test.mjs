...
test("hosted runs use durable storage, Vercel Workflow, real VideoDB and public abuse controls", async () => {
  const [route, starter, workflow, db, security, ui] = await Promise.all([
    readFile(resolve(frontendRoot, "routes/api/jobs.post.ts"), "utf8"),
    readFile(resolve(frontendRoot, "server/start-job.ts"), "utf8"),
  ...
  assert.match(route, /startDurableJob/);
  assert.match(starter, /start\(analyzeMatch/);
